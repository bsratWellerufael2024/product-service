...
export class Products {
  ...
  // ❌ REMOVE this column because @ManyToOne already defines categoryId
  // @Column()
  // categoryId: string;
  ...
  @JoinColumn({ name: 'categoryId', referencedColumnName: 'id' }) // Ensure it references `id`
  category: Category;

  // @OneToMany(() => ProductVariant, (variant) => variant.productId)
  // variants: ProductVariant[];
  @OneToMany(() => ProductVariant, (variant) => variant.product) // ✅ Fix: Use 'product' instead of 'productId'
  variants: ProductVariant[];
  @Column()
  cost_price: number;
